...
Move::Move(const char str[3]) :
		Move(str[1] - '0', str[0] - 'a') {
}
...
bool TicTacToeGame::doMove(Move move) {
	if (board[move.row][move.col] != ' ' || !move.isValid())
		return false;
...
Move TicTacToeGame::bestMove() {
	...
	for (int row = 0; row < 3; ++row) {
		for (int col = 0; col < 3; ++col) {
			Move m(row, col);
			if (!doMove(m))
				continue; // check all currently possible moves
	...
	case '\0':
		...
		for (int row = 0; row < 3; ++row) {
			for (int col = 0; col < 3; ++col) {
				...
				if (currentPlayer != 'x')
					val = std::max<double>(val, eval()); // x made the last move
				else
					val = std::min<double>(val, eval());
